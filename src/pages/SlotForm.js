...
import { FormRow, FormRowSelect, FormRowTime, Header } from "../components";
import { useDispatch, useSelector } from "react-redux";
import { handleSlotChange, updateSlot } from "../features/slot/slotSlice";
import { slotStatus } from "../data/dummy";
...
const SlotForm = () => {
  const {
    ...
    slotNumber,
    editSlotId,
  } = useSelector((store) => store.slots);
  const dispatch = useDispatch();

  const handleSlotInput = (e) => {
    const name = e.target.name;
    const value = e.target.value;
    dispatch(handleSlotChange({ name, value }));
  };
  const handleSubmit = (e) => {
    e.preventDefault();
    let tempStatus = parseInt(status);
    let tempStartTime = moment(startTime).toISOString(true);
    let tempEndTime = moment(endTime).toISOString(true);
  ...
  return (
    <div className="m-2 md:m-10 p-2 md:p-10 bg-gray-100 rounded-3xl">
      <Header category="Page" title="Edit Slot" />
      <form>
        {/* Field */}
        <FormRow
          type="text"
